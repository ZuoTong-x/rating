...
评分提交会在同一事务中更新任务并写入 `task_stats_events`。后台默认每 2 秒消费事件，增量更新项目、打分人、行为风险和完成时段统计。服务不再周期性扫描活跃项目的全部任务；项目生成、重新分配和特殊回退仍会按项目执行精确统计重建。已处理事件默认保留 48 小时，并由独立的每小时清理任务删除。
